...
import { getFreeLesson } from '@/actions/lesson.action'
...
function Hero(course: ICourse) {
	...
	const onHandler = async () => {
		if (lessons.length) return setOpen(true)
				...
				<DialogContent className='custom-scrollbar max-h-full max-w-full overflow-y-auto md:max-w-4xl'>
					...
					<div className='flex flex-col'>
						{lessons.map(item => (
							<div
								key={item._id}
								className='flex cursor-pointer items-center justify-between gap-2 border-t 
